...
def generate_pdf(report: Report) -> bytes:
    try:
        ...
        # --- DATA EXTRACTION ---
        sections = {}
        tagline = "AI-Powered Personalized Platform"
        advanced_metrics = {}
        try:
            if report.full_report:
                import json
                raw_data = json.loads(report.full_report)
                sections = raw_data.get("sections", {})
                tagline = raw_data.get("tagline", tagline)
                advanced_metrics = raw_data.get("advanced_metrics", {})
        except Exception:
            pass
        ...
        flowables.append(Paragraph(f'"{tagline}"', custom_subtitle))
        
        flowables.append(Spacer(1, 100))
        flowables.append(Paragraph(f"<b>Generated by:</b> AgentFlow AI", ParagraphStyle('Centered', parent=custom_normal, alignment=TA_CENTER)))
        flowables.append(Paragraph(f"<b>Date & Time:</b> {datetime.datetime.now().strftime('%B %d, %Y - %I:%M %p')}", ParagraphStyle('Centered', parent=custom_normal, alignment=TA_CENTER)))
        flowables.append(Paragraph(f"<b>Report ID:</b> {report.id}", ParagraphStyle('Centered', parent=custom_normal, alignment=TA_CENTER)))
        ...
        # --- ADVANCED METRICS (KPI Cards Simulation) ---
        if advanced_metrics:
            flowables.append(Paragraph("Advanced Evaluation Metrics", custom_heading))
            for key, val in advanced_metrics.items():
                flowables.append(Paragraph(f"<b><font color='#F43F5E'>{key}:</font></b> {val}", custom_normal))
            flowables.append(Spacer(1, 10))
